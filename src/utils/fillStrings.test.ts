import { fillStrings } from "./fillStrings";
...
describe("strings with duplicate characters", () => {
  ...
  it("Does not fill strings that are totally different.", () => {
    const [arr1, arr2] = fillStrings("abc", "xyz12");

    expect(arr1).toEqual(["a", "b", "c", null, null]);
    expect(arr2).toEqual(["x", "y", "z", "1", "2"]);
  });
});
